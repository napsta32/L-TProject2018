...
object ScalaJSExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val group: Selection[ChordGroup] = g.append("g").attr("class", "groups")
      .selectAll("g")
      .data((c: ChordArray) => c.groups)
      .enter().append("g")

    // implicit def fun2fun[T, U](x: (T) => U): js.Function3[T, Int, UndefOr[Int], U] = (v: T, _: Int, _: UndefOr[Int]) => x(v)
    // implicit def fun2nativefun[T](x: (T) => Color): js.Function3[T, Int, UndefOr[Int], Primitive] = (v: T, _: Int, _: UndefOr[Int]) => x(v).toString()


    // var fillStyle: js.Function3[ChordGroup, Int, UndefOr[Int], Primitive] = (d: ChordGroup) => color(d.index)
